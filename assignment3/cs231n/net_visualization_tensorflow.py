...
def compute_saliency_maps(X, y, model):
    ...
    N, *_ = X.shape
    X = tf.convert_to_tensor(X, tf.float32, name='X')
    ...
    dz_dx = t.gradient(correct_scores, X).numpy()
    dz_dx = np.max(np.abs(dz_dx), axis = -1)
...
def make_fooling_image(X, target_y, model):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    X_fooling = tf.convert_to_tensor(X_fooling, tf.float32, name='X_fooling')
    for i in range(10000):
        with tf.GradientTape() as t:
            t.watch(X_fooling)
            scores = model.call(X_fooling)
            grad = t.gradient(scores[0,target_y], X_fooling)
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as t:
        t.watch(X)
        scores = model.call(X)
        grad = t.gradient(scores[:, target_y], X) - 2 * l2_reg * X
