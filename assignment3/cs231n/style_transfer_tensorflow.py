...
def tv_loss(img, tv_weight):
    ...
    a = tf.reduce_sum((img[:,1:,:,:] - img[:,:-1,:,:])**2)
    b = tf.reduce_sum((img[:,:,1:,:] - img[:,:,:-1,:])**2)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    # for idx, weight in enumerate(style_weights):
    for idx in range(len(style_layers)):
        # print(feats[layer].shape)
        # print(style_targets[layer].shape)
        # idx = style_layers[idx]
        layer = style_layers[idx]
        gram_1 = gram_matrix(feats[layer])
        # gram_2 = gram_matrix(style_targets[layer])
        # print(tf.shape(gram_1), tf.shape(style_targets[idx]))
        loss += tf.constant(style_weights[idx]*1.0) * tf.reduce_sum((gram_1 - style_targets[idx])**2)
...
def gram_matrix(features, normalize=True):
    ...
    if normalize:
        gram = tf.divide(gram, tf.cast(H*W*C, dtype=tf.float32))
...
def content_loss(content_weight, content_current, content_original):
    ...
    tmp = tf.subtract(content_current, content_original)
    loss =  tf.constant(content_weight) * tf.reduce_sum(tf.pow(tmp, 2))
    return loss
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
